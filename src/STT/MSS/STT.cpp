...
void solve() {
  ...
  for (int i = 1; i < (int)orig_num_nets; i++) {
  ...
  for (int i = 0; i + 1 < orig_num_nets; i++) {

    int w_v_i = 0;

    vector<int> dummy;
    for (int j = 0; j + 1 < orig_num_nets; j++)
      if (closestV[j].s != closestV[i].s) {
        int dX = abs(X[closestV[j].s] - X[closestV[i].s]),
            dY = abs(Y[closestV[j].s] - Y[closestV[i].s]);
        dummy.push_back(dX + dY);
      }
    sort(begin(dummy), end(dummy));

    for (int k = 0; k < min((int)dummy.size(), KNN); k++)
      w_v_i += dummy[k];
  ...
  for (int i = 0; i < (int)VP.size(); i++) {
    ...
    for (int j = i + 1; j < sz(VP); j++) {
      int dX = abs(X[VP[i].s] - X[VP[j].s]), dY = abs(Y[VP[i].s] - Y[VP[j].s]);
      smallestD.push_back(
  ...
  int ptr = 0;
  while (ptr < (int)smallestD.size() && (int)candidates.size() > LAMBDA) {
  ...
  /* find the center most point by minimum manhattan distance to the center of
   * the bounding box. */
  ...
  for (int alp = 0; alp <= 10; alp++) {
    float a = (float)alp / 10;
    for (int i = 0; i < sz(seedCandidates); i++) {
      enterInput();
      getNearestNeighbors();
      ...
      fout << "\"normalizedWirelength\": " << (float)wl / nWL << ",\n";
      fout << "\"normalizedSkew\": " << (float)skew / nSkew << ",\n";
      fout << "\"seededEdges\": [" << seedCandidates[i] << "]\n";
      fout << "}" << (alp == 10 && i + 1 == sz(seedCandidates) ? "\n" : ",\n");
  ...
  for (int alp = 0; alp <= 10; alp++) {
    float a = (float)alp / 10;
    for (int i = 0; i < sz(seedCandidates); i++) {
      for (int j = i + 1; j < sz(seedCandidates); j++) {
        enterInput();
        getNearestNeighbors();
        ...
        fout << "\"seededEdges\": [" << seedCandidates[i] << ","
             << seedCandidates[j] << "]\n";
        fout << "}"
             << (alp == 10 && i == sz(seedCandidates) - 2 &&
                         j == sz(seedCandidates) - 1
                     ? "\n"
                     : ",\n");
      }
    }
  ...
  for (int alp = 0; alp <= 10; alp++) {
    ...
    for (int i = 0; i < sz(seedCandidates); i++) {
      for (int j = i + 1; j < sz(seedCandidates); j++) {
        for (int k = j + 1; k < sz(seedCandidates); k++) {
          enterInput();
          getNearestNeighbors();
          ...
          fout << "\"normalizedSkew\": " << (float)skew / nSkew << ",\n";
          fout << "\"seededEdges\": [" << seedCandidates[i] << ","
               << seedCandidates[j] << "," << seedCandidates[k] << "]\n";
          fout << "}"
               << (alp == 10 && i == sz(seedCandidates) - 3 &&
                           j == sz(seedCandidates) - 2 &&
                           k == sz(seedCandidates) - 1
                       ? "\n"
                       : ",\n");
        }
      }
...
int main(int argc, char *argv[]) {
  ...
  LAMBDA = atoi(argv[6]), KNN = atoi(argv[8]);
  cout << LAMBDA << " " << KNN << "\n";
  int T;
  fin >> T;
  fout << "[\n"
