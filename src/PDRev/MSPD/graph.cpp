...
Graph::Graph(int _num_terminals, int _verbose, float _alpha1, float _alpha2,
             float _alpha3, float _alpha4, int _root, float _beta,
             float _margin, int seed1_, int seed2_, int seed3_, int _distance,
...
/*
   ...
   */
int dist(Node &p, Node &q) { return abs(p.x - q.x) + abs(p.y - q.y); }
...
int Graph::ComputeWL(int cIdx, int pIdx, bool isRemove, int eShape) {
  ...
  switch (IdentLoc(pIdx, cIdx)) {
  case 0:
    WL += !eShape ? DeltaS(pIdx, cIdx, isRemove) : DeltaW(pIdx, cIdx, isRemove);
    WL += !eShape ? DeltaE(cIdx, pIdx, isRemove) : DeltaN(cIdx, pIdx, isRemove);
    break;
  case 1:
    WL += !eShape ? DeltaS(pIdx, cIdx, isRemove) : DeltaE(pIdx, cIdx, isRemove);
  ...
  case 2:
    ...
    break;
  case 3:
    WL += !eShape ? DeltaN(pIdx, cIdx, isRemove) : DeltaW(pIdx, cIdx, isRemove);
    WL += !eShape ? DeltaE(cIdx, pIdx, isRemove) : DeltaS(cIdx, pIdx, isRemove);
    break;
  case 4:
    WL += !eShape ? DeltaN(pIdx, cIdx, isRemove) : DeltaE(pIdx, cIdx, isRemove);
...
void Graph::refineSteiner2() {
  ...
  for (int i = dag.size() - 1; i >= 0; --i) {
    ...
    if (verbose > 3)
      cout << "cN: " << cN << endl;
    if (cN.idx == 0 || cN.idx == seed1 || cN.idx == seed2 || cN.idx == seed3) {
      continue;
    }
...
bool Graph::buildNearestNeighbors_single_node(int num_terms, int node_idx) {
...
bool Graph::buildNearestNeighborsForSPT(int num_terms) {
...
int Graph::calc_tree_skew() {

  int mn = -1, mx = -1;
  for (int v = 1; v < orig_num_terminals; v++) {
    int vToRoot = 0;
    int at = v;
    while (at != 0) {
      int par = nodes[at].parent;
      vToRoot += dist(nodes[at], nodes[par]);
      at = par;
    }
    if (mn == -1 || vToRoot < mn)
      mn = vToRoot;
...
bool Graph::run_PD_brute_force(float alp) {
  ...
  if (seed1) {
    heap_insert(seed1, 0);
    nodes[seed1].parent = root_idx;
    nodes[seed1].path_length = 0;
    seedCnt++;
  }
  if (seed2) {
    heap_insert(seed2, 0);
    nodes[seed2].parent = root_idx;
    nodes[seed2].path_length = 0;
    seedCnt++;
  }
  if (seed3) {
    heap_insert(seed3, 0);
    nodes[seed3].parent = root_idx;
    nodes[seed3].path_length = 0;
    seedCnt++;
  }
       ...
       k++) /* n points to be extracted from heap */
  {
    ...
    if (seedCnt) {
      if (i > 0) {
        ...
        // BruteForcefor(int oct = 0;  oct < num_terminals;  oct++ )
...
void Graph::PDBU_new_NN() {
  ...
  while ((tree_cost_difference > 0) || (tree_cost_difference < -1)) {
    ...
    for (int j = 0; j < num_terminals; ++j) /* For each terminal */
    {
      ...
      for (int iter = 0; iter <= distance; iter++) {
        ...
        for (int idx = 0; idx < nodes[j].swap_space[iter].size(); idx++) {
          ...
          // if ((seed1 && node_e_new == seed1) || (seed2 && node_e_new ==
          // seed2) || (seed3 && node_e_new == seed3)) isSeed = 1;
          for (int oct = 0; oct < nn[node_e_new].size(); oct++) {
              ...
              while (par != j) {
                ...
                cumul_det_cost_term += (nodes[par].K_t - nodes[child].K_t) *
                                       (nodes[nn_node].det_cost_node +
                                        sum_q_terms - nodes[par].det_cost_node);
                child = par;
                par = nodes[par].parent;
                if ((child == 0 && par == 0) || (child == seed1 && par == 0) ||
                    (child == seed2 && par == 0) ||
                    (child == seed3 && par == 0))
                  break;
              }
            ...
            bool isSeed = 0;
            if (seed1 && node_e_new == seed1)
              isSeed = 1;
            if (seed2 && node_e_new == seed2)
              isSeed = 1;
            if (seed3 && node_e_new == seed3)
              isSeed = 1;
            if (!isSeed && swap_cost < overall_min_cost) {
    ...
    if (overall_min_cost < -0.05) {
      ...
      swap_and_update_tree(overall_min_node, overall_min_nn_idx, distance,
                           overall_initial_i);
    }
...
void Graph::constructSteiner() {
  ...
  for (int i = 1; i < dag.size(); ++i) {
    ...
    if (cIdx == seed1 || cIdx == seed2 || cIdx == seed3) {
      continue;
    }
    ...
    for (int j = 0; j < e.STNodes.size(); ++j) {
      ...
      if (idx == 0) {
        ...
        for (int k = 0; k < cN.sp_chil.size(); ++k) {
          if (pN.idx == cN.sp_chil[k] && nodes[pN.idx].parent != 0) {
            flagIdx = false;
       ...
       k--) // Starting from the nodes in the second level from bottom
  {
    for (int l = 0; l < tree_struct[k].size(); l++) {
      ...
      if (set_of_nodes.size() > 2) {
        if (!done.count(child)) {
          // cout << "Working on: " << nodes[child].idx << "\n";
          done.insert(child);
  ...
  if (verbose > 1) {
    for (int j = 0; j < num_terminals; ++j) /* For each terminal */
    {
      ...
      if (nodes[j].children.size() > 0) {
        for (int i = 0; i < edges[j].lower_best_config.size(); i = i + 2)
          cout << "\t[Lower]Child=Node" << edges[j].lower_best_config[i]
               << " its best shape=" << edges[j].lower_best_config[i + 1]
               << " ; ";
        cout << endl;
        for (int i = 0; i < edges[j].upper_best_config.size(); i = i + 2)
    ...
    int wl_val = calc_tree_wl_pd(), ov_val = edges[0].best_ov;
...
bool Graph::fix_max_dc() {
  ...
  if (verbose > 2) {
    ...
    for (int k = 0; k < tree_struct_1darr.size(); k++)
  ...
  for (int k = 1; k < tree_struct_1darr.size(); k++) {
    ...
    if (cnode == seed1 || cnode == seed2 || cnode == seed3)
      continue;
      ...
      if (min_dc_nn != -1) {
        ...
        while (chi.size() != 0) {
          ...
          for (id = 0; id < tmp.size(); id++) {
            ...
            count++;
            chi.insert(chi.end(), nodes[tmp[id]].children.begin(),
...
bool Graph::get_overlap_lshape(vector<Node> &set_of_nodes, int index) {
  ...
  int best_sps_curr_node_idx_x = 9999999, best_sps_curr_node_idx_y = 9999999;
  // int best_sps_curr_node_idx_x=INT_MAX, best_sps_curr_node_idx_y=INT_MAX;
  ...
  if ((dont_care_flag != 0) && (dont_care_flag == max_ap_cnt - 1)) {
    ...
    copy_if(List1.begin(), List1.end(), back_inserter(dont_care_child),
            [&not_dont_care_child](const int &arg) {
              return (find(not_dont_care_child.begin(),
                           not_dont_care_child.end(),
                           arg) == not_dont_care_child.end());
            });
...
int Graph::calc_overlap(vector<vector<Node>> &set_of_nodes) {
  ...
  for (int i = 0; i < set_of_nodes.size(); i++) {
    for (int j = i + 1; j < set_of_nodes.size(); j++) {
      ...
      tmp_ov = (int)boost::geometry::length(output);

      // Known Problem - output-double, when converting to int for output_nodes,
      // 232 becomes 231 for somereason
      vector<Node> output_nodes;
      for (int g = 0; g < output.size(); g++)
        output_nodes.push_back(Node(0, output[g].x(), output[g].y()));
...
int Graph::calc_ov_x_or_y(vector<Node> &sorted, Node curr_node, char tag[]) {
  ...
  if (ind_of_curr_node > 0) {
    ...
    cnt = 0;
    int s = tmp.size(); // cout << "Size of tmp = " << s << endl;
    for (int j = 0; j < s; j++) {
      tmp_ov.push_back(0);
      tmp_ov[j] = tmp[j] * (s - j);
    }
...
void Graph::swap_and_update_tree(int min_node, int nn_idx, int distance,
                                 int i_node) {
  int child = min_node, par = nodes[child].parent;
  while ((par != i_node) &&
         (par != 0 && par != seed1 && par != seed2 && par != seed3)) {
